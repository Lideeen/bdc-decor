	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		if(empty($_POST['full_name']) || empty($_POST['name']) || empty($_POST['height']) || empty($_POST['width']) || empty($_POST['length']) ||
           ...
           !$_FILES['images']['tmp_name'][0]){
			...
			$sql = "insert into plintus_potolochny values(null, '$full_name', '$name', '$height', '$width', '$length', '$price', '$inst_price', 
			...
			if($result){
				if(mkdir("images/".$count, 0777)){
					...
					$sql = "delete from plintus_potolochny where images_dir='$count'";
					$conn->query($sql);
