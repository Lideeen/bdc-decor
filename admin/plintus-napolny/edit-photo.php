<?php
    ...
    if(isset($_POST['image'])) {
        $image = $_POST['image'];
        $sql = "select description, position, image from plintus_napolny_collection where id='$image'";
    ...
    if(isset($_POST['id'])){
        ...
        if(move_uploaded_file($_FILES['userfile']['tmp_name'], $target_file)) {
            $sql = "select image from plintus_napolny_collection where id='$id'";
            $result = $conn->query($sql);
            if($result){
                while($row = $result->fetch_assoc()){
                ...
                $sql = "update plintus_napolny_collection set image='$image_collection' where id='$id'";
                $result = $conn->query($sql);
            }
        }
    }
