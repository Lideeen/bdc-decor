<?php
    ...
    if($dir == "no"){
        $type = $_POST['type'];
        if($type != 7) {
            $sql = "select image from content where category=5 and type='$type'";
            $result = $conn->query($sql);
            while ($row = $result->fetch_assoc()) {
                $imageDelete = $row['image'];
            }
            unlink('images/' . $imageDelete);
            $sql = "update content set image='{$_FILES['userfile']['name']}' where category=5 and type='$type'";
            $conn->query($sql);
            copy($_FILES['userfile']['tmp_name'], "images/" . $_FILES['userfile']['name']);
        }else{
            $sql = "select image from reference where id_category=4";
            $result = $conn->query($sql);
            while ($row = $result->fetch_assoc()) {
                $imageDelete = $row['image'];
            }
            unlink('images/' . $imageDelete);
            $sql = "update reference set image='{$_FILES['userfile']['name']}' where id_category=5";
            $conn->query($sql);
            copy($_FILES['userfile']['tmp_name'], "images/" . $_FILES['userfile']['name']);
        }
    }else {
